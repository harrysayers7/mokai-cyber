...
export default function Dashboard() {
  // Mock data for MVP demo - in production this will come from database
  ...
  const overallMaturity = Math.round(
    controls.reduce((sum, control) => sum + control.maturityLevel, 0) / 8 * 100 / 3
  )
  ...
  const updateMaturityLevel = (controlId: string, newLevel: string) => {
    const level = parseInt(newLevel)
    setControls(prev => 
      prev.map(control => 
        control.id === controlId 
          ? { ...control, maturityLevel: level, lastUpdated: new Date() }
          : control
      )
    )
  }
    ...
    <div className="container mx-auto p-6 min-h-screen bg-gray-50">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          {mockOrganization.name} • ABN: {mockOrganization.abn}
        </p>
      </div>
          ...
          <Card key={control.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="text-sm text-gray-500">
                  Last updated: {control.lastUpdated.toLocaleDateString()}
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardContent className="pt-4 text-center">
            <div className="text-2xl font-bold text-green-600">
              {controls.filter(c => c.maturityLevel === 3).length}
            </div>
        ...
        <Card>
          <CardContent className="pt-4 text-center">
            <div className="text-2xl font-bold text-yellow-600">
              {controls.filter(c => c.maturityLevel === 2).length}
            </div>
        ...
        <Card>
          <CardContent className="pt-4 text-center">
            <div className="text-2xl font-bold text-orange-600">
              {controls.filter(c => c.maturityLevel === 1).length}
            </div>
        ...
        <Card>
          <CardContent className="pt-4 text-center">
            <div className="text-2xl font-bold text-red-600">
              {controls.filter(c => c.maturityLevel === 0).length}
            </div>
